...
namespace WordReadPrg
{
    class Program
    {
           ...
           // {
                ...
                if (MyDataset.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (System.Data.DataRow MyRow in MyDataset.Tables[0].Rows)
                    {
                        ...
                        //for blob chnages for actual documents
                        byte[] bytes;
                        string filepath = "";
                        //for blob
                        string Connectionstring = pg.getProjectConnectionString(new Guid(pg.GetProjectUID(new Guid(MyRow["DocumemtUID"].ToString()))));
                        //
                        DataSet docBlob = pg.GetActualDocumentBlob_by_ActualDocumentUID(new Guid(MyRow["DocumemtUID"].ToString()),Connectionstring);
                        if (docBlob.Tables[0].Rows.Count > 0)
                        {
                            bytes = (byte[])docBlob.Tables[0].Rows[0]["Blob_Data"];
                            if (MyRow["Doc_path"].ToString().Contains(".docx"))
                            {
                                filepath = AppDomain.CurrentDomain.BaseDirectory + "\\Encryptpath\\" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".docx";
                                fileSavePath = AppDomain.CurrentDomain.BaseDirectory + "\\Encryptpath\\" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".docx";
                            }
                            else
                            {
                                filepath = AppDomain.CurrentDomain.BaseDirectory + "\\Encryptpath\\" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".doc";
                                fileSavePath = AppDomain.CurrentDomain.BaseDirectory + "\\Encryptpath\\" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".doc";
        ...
        //adding on 03/05/2023
        public DataSet GetActualDocumentBlob_by_ActualDocumentUID(Guid ActualDocumentUID, string ConnectionString)
        {
        ...
        public string GetProjectUID(Guid ActualDocumentUID)
        {
            DataSet ds = new DataSet();
            ...
            {
                SqlConnection con = new SqlConnection(GetConnectionString());
                SqlDataAdapter cmd = new SqlDataAdapter("select * From ActualDocuments Where ActualDocumentUID='" + ActualDocumentUID + "'", con);
                cmd.Fill(ds);
