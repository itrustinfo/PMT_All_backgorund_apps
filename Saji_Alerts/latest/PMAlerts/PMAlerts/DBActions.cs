...
namespace PMAlerts
{
    public class DBActions
    {
        ...
        public int InsertAlerts(Guid Alert_UID, Guid TaskUID, string Alert_Text, DateTime Alert_Date, Guid Alert_User, Guid ProjectUID, Guid WorkPackageUID, string Status,DateTime RestrictedDate,string NotifType)
        {
            int cnt = 0;
            try
            {
                using (SqlConnection con = new SqlConnection(GetConnectionString()))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("usp_InsertAlerts"))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@RestrictedDate", RestrictedDate);
