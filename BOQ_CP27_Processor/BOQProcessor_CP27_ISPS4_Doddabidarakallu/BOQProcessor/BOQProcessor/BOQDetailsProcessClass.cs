...
namespace BOQProcessor
{
    class BOQDetailsProcessClass
    {
        ...
        // static string connectionString = @"server=localhost\SQLEXPRESS2019;Database=ONTB_Server; user id=sa;password=itrust@123;";
        //static string connectionString = @"server=localhost;Database=ONTB-29-April; user id=sa;password=Itrust@123;";
                ...
                using (OleDbConnection con = new OleDbConnection(conn))
                {
                    try
                    {
                        ...
                        foreach (DataRow dr in dtexcel.Rows)
                        {
                            ...
                            {
                                if (dr.ItemArray[1].ToString().TrimEnd() == "Item No.")
                                {
                                    //for (int i = 0; i < dr.ItemArray.Length; i++)
                                    //{
