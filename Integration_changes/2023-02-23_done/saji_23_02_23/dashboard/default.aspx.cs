        ...
        protected void DDlProject_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (DDlProject.SelectedValue != "--Select--")
            {
                ...
                //ds = getdt.GetWorkPackages_By_ProjectUID(new Guid(DDlProject.SelectedValue));
                    ...
                    else if (Request.QueryString["Option"] != null)
                    {
...
vAxes: {
        ...
        public static string  GetDetails(string Id)
        {
            if (WebConfigurationManager.AppSettings["Domain"] != "Suez")
            {
                if (HttpContext.Current.Session["TypeOfUser"].ToString() != "U")
                {
                    string[] selectedValue = HttpContext.Current.Session["Project_Workpackage"].ToString().Split('_');
            ...
            if (DDlProject.SelectedValue != "")
            {
                ...
                if (dtTasks.Rows.Count > 0)
                {
                    ...
                    for (int cnt = 3; cnt < dtTasks.Columns.Count; cnt++)
                    {
                        ...
                        if (cnt == 3)
                        {
                            lblTotalM.Text = columnTotal.ToString();
                        }
                        else if (cnt == 4)
                        {
                            lblExecutedM.Text = columnTotal.ToString();
                        }
                        else if (cnt == 5)
                        {
                            lblBalanceM.Text = columnTotal.ToString();
                        }
                    }
        ...
        private void Bind_LabourChart()
        {
            if (DDLWorkPackage.SelectedValue != "--Select--")
            {
                ...
                decimal AverageLabour = getdt.GetLabourDashboardGraph(new Guid(DDLWorkPackage.SelectedValue), "Labour");

                AverageLabour = Math.Round(AverageLabour, 1);

                //string[] YearMonth = new string[2];

                //if (ViewState["YearMonth"] != null)
                //    YearMonth = ViewState["YearMonth"].ToString().Split(',');
                ...
                strScript.Append("['" + ViewState["LabourMonth"].ToString() + "', " + AverageLabour + ", '#3366CC', '" + CurrencySymbol + ' ' + AverageLabour + "']]);");
                strScript.Append(@"var options = {
        ...
        private void Bind_DocumentsChart5()
        {
            if (DDLWorkPackage.SelectedValue != "--Select--")
            {
                ...
                function drawBasic() {
                    ...
                    BtoF.HRef = "/_content_pages/dashboard/default.aspx?option=" + Request.QueryString["Option"].ToString() + "&selection=2&back=1";
                    BtoF.Visible = true;
                }
                else
                {
