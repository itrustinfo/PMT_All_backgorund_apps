...
namespace WordReadPrg
{
    class Program
    {
                        ...
                        try
                        {
                           ...
                           // fileSavePath = MyRow["Doc_path"].ToString();// @"C:\PMT\ProjectManagementTool\ProjectManagementTool\RegExcel\Audit - 06 April 2020.docx";  //  Server.MapPath("~") + Request.QueryString["file_path"];// Server.MapPath("~/Temp/") + Path.GetFileName(FileUpload1.PostedFile.FileName);
                            //for blob chnages for actual documents
                             byte[] bytes;
                        string filepath = "";
                           DataSet docBlob = pg.GetActualDocumentBlob_by_ActualDocumentUID(new Guid(MyRow["DocumemtUID"].ToString()));
        ...
        public DataSet GetActualDocumentBlob_by_ActualDocumentUID(Guid ActualDocumentUID)
        {
            ...
            SqlConnection con = new SqlConnection(pg.GetConnectionString());
            try
            {
