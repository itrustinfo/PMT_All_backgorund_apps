    ...
    public partial class Form1 : Form
    {
        ...
        private void StartbackUP()
        {
            ...
            try
            {
                ...
                string backupfileName = txtBckfileName.Text + "_" + DateTime.Now.ToString("dd_MM_yyyy") + ".bak";
               ...
               // string deletedestpath = string.Empty;
                ...
                lstBoxLog.Items.Add("Application backUp Started  at " + DateTime.Now);
                ...
                {
                    string[] originalFiles = Directory.GetFiles(sourcePath, "*", System.IO.SearchOption.AllDirectories);
                   
                   
                    Array.ForEach(originalFiles, (originalFileLocation) =>
                    {
                        FileInfo originalFile = new FileInfo(originalFileLocation);
                        FileInfo destFileC = new FileInfo(originalFileLocation.Replace(sourcePath, destPath));
                       //
                        if (originalFile.CreationTime >= curentdate || originalFile.LastWriteTime >= curentdate)
                        {
                            if (!destFileC.Exists)
                            {
                                Directory.CreateDirectory(destFileC.DirectoryName);
                                originalFile.CopyTo(destFileC.FullName, true);
                            }
                            else
                            {
                                originalFile.CopyTo(destFileC.FullName, true);
                ...
                lstBoxLog.Items.Add("Application backUp Finished  at " + DateTime.Now);
                lstBoxLog.Items.Add("BackUp Diretory  at " + destPath);
              
                lstBoxLog.Items.Add("BackUp Complete !");
                lstBoxLog.Refresh();
                ...
                testfile.Close();
                // write to db
                StoreBackupLogs(starttime, endtime, backuptype, Odd_evenPath);
            ...
            catch (Exception ex)
            {
                ...
                if (ErrorEmailTo != "")
                {
                    StoreEmaildataToMailQueue(MailUID, MailUID, dtemailCred.Rows[0][0].ToString(), ErrorEmailTo, "Documents Uploaded !", " :-Error occurred: " + ex.Message, ErrorCCTo, "");
                }
                //
...
" VALUES (@ServerName,@Username,@Password,@DatabaseName,@DB_backupFilename,@SourceAppPath,@DestinationPath,@DestinationDBPath)";
    ...
    " VALUES (@ServerName,@Username,@Password,@DatabaseName,@DB_backupFilename,@SourceAppPath,@DestinationPath)";
        ...
        private bool isFullbackupPresent()
        {
            bool sresult = false;
            DateTime currentday = DateTime.Now.AddDays(-6);
            for(int i=0; i<=5;i++)
            {
                if((int)currentday.DayOfWeek == Fullbackupday)
                {
                    ...
                    return isfullBackuppresent(currentday);
                }
                else
                {
                    currentday = currentday.AddDays(1);
        ...
        private bool isfullBackuppresent(DateTime sdate)
        {
            bool sresult = false;
            SqlConnection con = new SqlConnection(GetConnectionString());
            try
